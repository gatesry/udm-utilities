...
while [ ! -f $CONMON_DL ]; do
  curl -fsSLo $CONMON_DL https://github.com/gatesry/udm-utilities/blob/master/podman-update/bin/usr/libexec/podman/conmon?raw=true
  sleep 1
done
chmod +x $CONMON_DL
...
if [ ! -f $PODMAN_DL ]; then
  curl -fsSLo $PODMAN_DL https://github.com/gatesry/udm-utilities/blob/master/podman-update/bin/usr/bin/podman?raw=true
fi
chmod +x $PODMAN_DL
...
if [ ! -f $RUNC_DL ]; then
  curl -fsSLo $RUNC_DL https://github.com/gatesry/udm-utilities/blob/master/podman-update/bin/usr/bin/runc?raw=true
fi
chmod +x $RUNC_DL
...
if [ ! -f $SECCOMP ]; then
  ...
  curl -fsSLo $SECCOMP https://raw.githubusercontent.com/gatesry/udm-utilities/master/podman-update/bin/usr/share/containers/seccomp.json
fi
